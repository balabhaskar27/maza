...
bool CWallet::SignTransaction(CMutableTransaction &tx)
{
    ...
    for (const auto& input : tx.vin) {
        ...
        if (!ProduceSignature(TransactionSignatureCreator(this, &txNewConst, nIn, amount, SIGHASH_ALL), scriptPubKey, sigdata)) {	// if (!ProduceSignature(TransactionSignatureCreator(this, &txNewConst, nIn, amount, SIGHASH_ALL | SIGHASH_FORKID), scriptPubKey, sigdata)) {Maza: Replay attack protection
            return false;
        }
    ...
    unsigned int nBytes;
    {
        ...
        if (sign)
        {
            ...
            for (const auto& coin : setCoins)
            {
                ...
                if (!ProduceSignature(TransactionSignatureCreator(this, &txNewConst, nIn, coin.txout.nValue, SIGHASH_ALL), scriptPubKey, sigdata))     // if (!ProduceSignature(TransactionSignatureCreator(this, &txNewConst, nIn, coin.txout.nValue, SIGHASH_ALL | SIGHASH_FORKID), scriptPubKey, sigdata))Maza: Include fork ID in the signature
                {
                    strFailReason = _("Signing transaction failed");
                    return false;
                } else {
