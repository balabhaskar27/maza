...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "                             The recipient will receive less maza than you enter in the amount field.\n"
...
// Maza: Hive: Get current bee cost
...
// Maza: Hive: Get network hive info
...
// Maza: Hive: Return BCT tx id for a honey transaction in this wallet
...
// Maza: Hive: Return hive info for a single BCT
...
// Maza: Hive: Get wallet's own hive info
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" \"address_type\" )\n"
            ...
            "}\n"
            "\nResult (DEPRECATED. To see this result in v0.16 instead, please start mazad with -deprecatedrpc=addmultisigaddress).\n"
...
UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("addwitnessaddress")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "addwitnessaddress is deprecated and will be fully removed in v0.17. "
            "To use addwitnessaddress in v0.16, restart mazad with -deprecatedrpc=addwitnessaddress.\n"
...
void ListTransactions(CWallet* const pwallet, const CWalletTx& wtx, const std::string& strAccount, int nMinDepth, bool fLong, UniValue& ret, const isminefilter& filter)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        for (const COutputEntry& s : listSent)
        {
            ...
            // Maza: Hive: Sent transactions are never honey
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        for (const COutputEntry& r : listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                // Maza: Hive: Indicate whether this is honey (hive block coinbase tx)
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    // Maza: Hive: Include ishoney in documentation
...
// Maza: Hive: Callback to set the wallet back to unlocked only for hive on walletpassphrase timeout
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending maza\n"
            "\nArguments:\n"
	...
	// Maza: Hive: Support locked wallets
...
// Maza: Hive: Unlock wallet for hiving only (Prevent trivial sendmoney attack if user OS account compromised)
UniValue walletpassphrasehiveonly(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
			...
			"private keys such as sending maza, is not enabled and will require you to run\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    StartShutdown();
    return "wallet encrypted; Maza server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending maza.\n"
    ...
    if (!request.params[1].isNull()) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid maza address");
            }
...
{ //  category              name                        actor (function)           argNames
    //  --------------------- ------------------------    -----------------------  ----------
    { "wallet",             "getbeecost",               &getbeecost,               {"height"} },                                            // Maza: Hive: Get bee cost for given height (defaults to tipheight)
    { "wallet",             "createbees",               &createbees,               {"bee_count","community_contrib","honey_address"} },     // Maza: Hive: Create bee(s)
    { "wallet",             "gethiveinfo",              &gethiveinfo,              {"include_dead","min_honey_confirms"} },                 // Maza: Hive: Get current hive info
    { "wallet",             "getnetworkhiveinfo",       &getnetworkhiveinfo,       {"include_graph"} },                                     // Maza: Hive: Get current bee populations across whole network
    { "wallet",             "getbeecreationtxid",       &getbeecreationtxid,       {"honey_txid"} },                                        // Maza: Hive: Return BCT tx id for a honey transaction in this wallet
    { "wallet",             "getbctinfo",               &getbctinfo,               {"bct_txid","min_honey_confirms"} },                     // Maza: Hive: Return hive info for a single BCT
