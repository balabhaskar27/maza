...
void CreateCreditAndSpend(const CKeyStore& keystore, const CScript& outscript, CTransactionRef& output, CMutableTransaction& input, bool success = true)
{
    ...
    bool ret = SignSignature(keystore, *output, inputm, 0, SIGHASH_ALL | SIGHASH_FORKID);   // Maza: Replay attack protection
    assert(ret == success);
...
BOOST_AUTO_TEST_CASE(test_big_witness_transaction) {
    ...
    sigHashes.push_back(SIGHASH_NONE | SIGHASH_FORKID | SIGHASH_ANYONECANPAY);			// Maza: Replay attack protection
    sigHashes.push_back(SIGHASH_SINGLE | SIGHASH_FORKID | SIGHASH_ANYONECANPAY);		// Maza: Replay attack protection
    sigHashes.push_back(SIGHASH_ALL | SIGHASH_FORKID | SIGHASH_ANYONECANPAY);			// Maza: Replay attack protection
    sigHashes.push_back(SIGHASH_NONE | SIGHASH_FORKID);									// Maza: Replay attack protection
    sigHashes.push_back(SIGHASH_SINGLE | SIGHASH_FORKID);								// Maza: Replay attack protection
    sigHashes.push_back(SIGHASH_ALL | SIGHASH_FORKID);									// Maza: Replay attack protection
