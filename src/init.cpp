...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-maxoutboundconnections=<n>", strprintf(_("Maximum number of automatic outgoing connections (default: %u)"), DEFAULT_MAX_OUTBOUND_CONNECTIONS));   // Maza: Parameterisation of max outbound connections
    ...
    // Maza: Hive: Mining optimisations
    ...
    // Maza: MinotaurX+Hive1.2: Allow switching of default pow algo via conf / command line, for miners that can't easily adjust their getblocktemplate calls
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if maza is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if maza is started while in a temporary directory.\n",
    ...
    connOptions.nMaxOutbound = std::min((int)gArgs.GetArg("-maxoutboundconnections", DEFAULT_MAX_OUTBOUND_CONNECTIONS), connOptions.nMaxConnections);    // Maza: Parameterisation of max outbound connections
    ...
    // Maza: Hive: Start the mining thread
#ifdef ENABLE_WALLET
