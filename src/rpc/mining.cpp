 ...
 */
// Maza: Hive: count hashes with dedicated function, dont use chainwork. GetNumHashes is Hive Aware.
// Maza: MinotaurX+Hive1.2: Only consider the correct powType when counting hashes
UniValue GetNetworkHashPS(int lookup, int height, POW_TYPE powType) {
    ...
    // Maza: MinotaurX+Hive1.2: Skip incorrect powType
	...
	arith_uint256 workDiff = GetNumHashes(*pb, powType);    // Maza: MinotaurX+Hive1.2: add powType param
    ...
    for (int i = 0; i < lookup; i++) {
        ...
        workDiff += GetNumHashes(*pb, powType);              // Maza: MinotaurX+Hive1.2: add powType param
    }
...
// Maza: Hive: Mining optimisations: Set hive mining params
...
// Maza: Hive: Mining optimisations: Get hive mining params
...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"errors\": \"...\"            (string) DEPRECATED. Same as warnings. Only shown when mazad is started with -deprecatedrpc=getmininginfo\n"
            "}\n"
            "\nExamples:\n"
    ...
    if (IsMinotaurXEnabled(chainActive.Tip(), Params().GetConsensus()))
        obj.push_back(Pair("minotaurxdifficulty", GetDifficulty(nullptr, false, POW_TYPE_MINOTAURX)));    // Maza: MinotaurX+Hive1.2
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        // Maza: MinotaurX+Hive1.2: Include help note about algo field in template_request
        throw std::runtime_error(
    ...
    std::string strAlgo = gArgs.GetArg("-powalgo", DEFAULT_POW_TYPE);   // Maza: MinotaurX+Hive1.2: Pow type (as string)
    if (!request.params[0].isNull())
    {
        ...
        // Maza: MinotaurX+Hive1.2: Override default pow type
    ...
    // Maza: MinotaurX+Hive1.2: Check for a valid pow type
    bool algoFound = false;
    POW_TYPE powType;
    for (unsigned int i = 0; i < NUM_BLOCK_TYPES; i++) {
        if (strAlgo == POW_TYPE_NAMES[i]) {
    ...
    static POW_TYPE lastPowType = NUM_BLOCK_TYPES;  // Maza: MinotaurX+Hive1.2
    if (pindexPrev != chainActive.Tip() ||
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 5) ||
        fLastTemplateSupportsSegwit != fSupportsSegwit ||
        lastPowType != powType) // Maza: MinotaurX+Hive1.2: Include powType check in cache refresh condition
    {
        ...
        pblocktemplate = BlockAssembler(Params()).CreateNewBlock(scriptDummy, fSupportsSegwit, nullptr, powType);   // Maza: MinotaurX+Hive1.2: Include powType
        lastPowType = powType;   // Maza: MinotaurX+Hive1.2: Cache pow type just requested
        if (!pblocktemplate)
...
UniValue estimatefee(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("estimatefee")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "estimatefee is deprecated and will be fully removed in v0.17. "
            "To use estimatefee in v0.16, restart mazad with -deprecatedrpc=estimatefee.\n"
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "mining",             "sethiveparams",          &sethiveparams,          {"hivecheckdelay", "hivecheckthreads", "hiveearlyout"} },  // Maza: Hive: Mining optimisations: Set hive mining params
    { "mining",             "gethiveparams",          &gethiveparams,          {} },  // Maza: Hive: Mining optimisations: Get hive mining params
};
