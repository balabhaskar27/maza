...
def main():
    ...
    tmpdir = "%s/maza_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and mazad must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "mazad"]) is not None:
            print("%sWARNING!%s There is already a mazad process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "MAZAD" not in os.environ:
        os.environ["MAZAD"] = build_dir + '/src/mazad' + exeext
        os.environ["MAZACLI"] = build_dir + '/src/maza-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `maza-cli help` (`rpc_interface.txt`).
